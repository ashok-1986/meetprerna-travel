...
The live deployment and repository are out of sync. The live page previously inspected contains only the hero, stays/gallery, and services sections; its `Let's talk` anchor points to a missing `#contact` target, and it exposes internal draft copy. The repository’s current `page.tsx` already contains an audience split, About/process section, footer, and contact section, but the form prevents submission and still uses placeholder contact destinations (`hello@example.com` and `wa.me/0000000000`). The plan must therefore treat deployment/version mismatch as a first-class release risk rather than assuming the repository is what users see.
